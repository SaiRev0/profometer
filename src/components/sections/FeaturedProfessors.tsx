  ...
  return (
    <section className='mt-8'>
      ...
      <div className='grid grid-cols-1 gap-6 md:grid-cols-2'>
        ...
        {!isLoadingLoved && (lovedData?.professors ?? []).length > 0 && (
            ...
            <CardContent className='space-y-4 p-3 pt-0 sm:p-6'>
              {isLoadingLoved
                ? Array.from({ length: 2 }).map((_, index) => (
                    <ProfessorCard
                      key={index}
                      professor={lovedData?.professors[0] as Professor}
                      isLoading={true}
                      variant='detailed'
                      showStars={false}
                    />
        ...
        {!isLoadingChallenging && (challengingData?.professors ?? []).length > 0 && (
            ...
            <CardContent className='space-y-4 p-3 pt-0 sm:p-6'>
              {isLoadingChallenging
                ? Array.from({ length: 2 }).map((_, index) => (
                    <ProfessorCard
                      key={index}
                      professor={challengingData?.professors[0] as Professor}
                      isLoading={true}
                      variant='detailed'
                      showStars={false}
                    />
