  ...
  return (
    <section className='mt-8'>
      ...
      <div className='grid grid-cols-1 gap-6 md:grid-cols-2'>
        {!isLoadingLoved && (lovedData?.courses ?? []).length > 0 && (
            ...
            <CardContent className='space-y-4 p-3 pt-0 sm:p-6'>
              {isLoadingLoved
                ? Array.from({ length: 2 }).map((_, index) => (
                    <CourseCard
                      key={index}
                      course={lovedData?.courses[0] as Course}
                      isLoading={true}
                      variant='detailed'
                    />
                  ))
                : lovedData?.courses.map((course) => (
        ...
        {/* Challenging Courses */}
        {!isLoadingChallenging && (challengingData?.courses ?? []).length > 0 && (
            ...
            <CardContent className='space-y-4 p-3 pt-0 sm:p-6'>
              {isLoadingChallenging
                ? Array.from({ length: 2 }).map((_, index) => (
                    <CourseCard
                      key={index}
                      course={challengingData?.courses[0] as Course}
                      isLoading={true}
                      variant='detailed'
                    />
                  ))
